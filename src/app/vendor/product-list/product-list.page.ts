...
export class ProductListPage {
  ...
  async loadData(event) {
    ...
    if (this.platform.is("hybrid"))
      await this.api.getItemIonic("products", this.filter).then(
        (res) => {
      ...
      await this.api.getItem("products", this.filter).subscribe(
        (res) => {
          this.tempProducts = res;
          this.products.push.apply(this.products, this.tempProducts);
          event.target.complete();
  ...
  getProduct(product) {
    ...
    this.navCtrl.navigateForward(
      this.router.url + "/view-product/" + product.id
    );
  }
  getCategory(id) {
  ...
  editProduct(product) {
    this.product.product = product;
    this.navCtrl.navigateForward(
      this.router.url + "/edit-product/" + product.id
    );
  }
    ...
    const alert = await this.alertController.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            if (this.platform.is("hybrid"))
              this.api.deleteItemIonic("products", product.id).then(
                (res) => {
                  this.getProducts();
                },
                (err) => {
              ...
              this.api.deleteItem("products", product.id).subscribe(
                (res) => {
                  this.getProducts();
                },
                (err) => {
