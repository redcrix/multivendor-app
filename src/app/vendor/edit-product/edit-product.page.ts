...
export class EditProductPage {
  ...
  async getProduct() {
    await this.api.getItem("products/" + this.id).subscribe(
      (res) => {
        this.product = res;
  ...
  async getVariationProducts() {
    await this.api
      .getItem("products/" + this.product.id + "/variations")
      .subscribe(
        (res) => {
          this.variations = res;
        },
  ...
  async getRelatedProducts() {
    if (this.product.related_ids != 0) {
      ...
      await this.api.getItem("products", filter).subscribe(
        (res) => {
  ...
  async getReviews() {
    await this.api
      .getItem("products/" + this.product.id + "/reviews")
      .subscribe(
        (res) => {
          this.reviews = res;
  ...
  async saveProduct() {
    ...
    if (this.platform.is("hybrid"))
      await this.api.putItemIonic("products/" + this.id, this.product).then(
        (res) => {
          this.product = res;
          this.productData.product = {};
      ...
      await this.api.put("products/" + this.id, this.product).subscribe(
        (res) => {
          this.product = res;
          this.productData.product = {};
          this.navCtrl.navigateBack("/tabs/account/vendor-products");
  ...
  reduceImages(selected_pictures: any): any {
    return selected_pictures.reduce((promise: any, item: any) => {
      return promise.then((result) => {
        return this.crop
          .crop(item, { quality: 75, targetHeight: 100, targetWidth: 100 })
  ...
  editProduct(product) {
    ...
    this.navCtrl.navigateForward(
      this.router.url + "/edit-variation-product/" + product.id
    );
  }
}
