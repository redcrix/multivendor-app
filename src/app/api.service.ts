...
export class ApiService {
  ...
  getAddonsList(endPoint, filter = {}) {
    const url = this.config.setUrl(
      "GET",
      "/wp-json/wc-product-add-ons/v1/" + endPoint + "?",
      filter
    );
    return this.http.get(url).pipe(
      tap((_) => {}),
  ...
  /**
   * Request for wc v2 apis
   */
  WCV2getItem(endPoint, filter = {}) {
    const url = this.config.setUrl(
      "GET",
      "/wp-json/wc/v2/" + endPoint + "?",
      filter
  ...
  WCV2getItemIonic(endPoint, filter = {}) {
    const url = this.config.setUrl(
      "GET",
      "/wp-json/wc/v3/" + endPoint + "?",
      filter
